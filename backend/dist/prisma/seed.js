...
const eligibilityService_1 = require("../src/services/eligibilityService");
...
async function main() {
    ...
    // Create Admin user
    const adminPasswordHash = await bcryptjs_1.default.hash('Admin@2026Password!', 10);
    ...
    for (const tc of testCandidates) {
        const { normalizedStatus } = eligibilityService_1.EligibilityService.normalizePaymentStatus(tc.paymentStatus);
        const eligible = eligibilityService_1.EligibilityService.calculateEligibility(normalizedStatus);
        candidatesData.push({
    ...
    for (let i = 1; i <= totalGenerate; i++) {
        ...
        const { normalizedStatus } = eligibilityService_1.EligibilityService.normalizePaymentStatus(rawPaymentStatus);
        const eligible = eligibilityService_1.EligibilityService.calculateEligibility(normalizedStatus);
        candidatesData.push({
            studentId,
            name,
